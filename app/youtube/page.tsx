...
export default function YouTubePage() {
  ...
  // 설교 상세 보기
  const [selectedSermon, setSelectedSermon] = useState<SermonDetail | null>(null)
  const [sermonDetailLoading, setSermonDetailLoading] = useState(false)
  ...
  // 설교 전체 텍스트 조회
  async function fetchSermonDetail(videoId: string) {
    setSermonDetailLoading(true)
    try {
      const res = await fetch(`/api/youtube/sermons/${videoId}`)
      const data = await res.json()
      if (data.success) {
        setSelectedSermon(data)
      } else {
        console.error('설교 상세 조회 실패:', data.error)
      }
    } catch (err) {
      console.error('설교 상세 조회 오류:', err)
    } finally {
      setSermonDetailLoading(false)
    }
  }
  ...
  return (
    <div className="relative flex flex-col min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto p-6">
              ...
              {processedSermons.map((sermon, idx) => (
                ...
                >
                  ...
                  <div className="ml-4 flex items-center gap-2">
                    <button
                      onClick={() => fetchSermonDetail(sermon.video_id)}
                      className="px-2 py-1 bg-amber-50 text-amber-600 text-xs rounded border border-amber-200 hover:bg-amber-100 transition-colors"
                    >
                      전체 보기
                    </button>
        ...
        {/* 설교 상세 보기 모달 */}
        {(selectedSermon || sermonDetailLoading) && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="flex-1 min-w-0">
                  {sermonDetailLoading ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-amber-800">설교 내용 불러오는 중...</span>
                    </div>
                  ) : selectedSermon?.sermon ? (
                    <>
                      <h3 className="text-lg font-bold text-amber-900 truncate">
                        {selectedSermon.sermon.videoTitle}
                      </h3>
                      <div className="flex items-center gap-3 mt-1 text-sm text-amber-700">
                        {selectedSermon.sermon.speaker && <span>{selectedSermon.sermon.speaker}</span>}
                        {selectedSermon.sermon.uploadDate && (
                          <span>• {new Date(selectedSermon.sermon.uploadDate).toLocaleDateString('ko-KR')}</span>
                        )}
                        {selectedSermon.sermon.sermonDuration && (
                          <span>• {Math.floor(selectedSermon.sermon.sermonDuration / 60)}분</span>
                        )}
                        <span>• {selectedSermon.totalChunks}개 청크</span>
                        <span>• {selectedSermon.textLength.toLocaleString()}자</span>
                      </div>
                    </>
                  ) : (
                    <span className="text-gray-500">설교 정보 없음</span>
                  )}
                </div>
                <button
                  onClick={() => setSelectedSermon(null)}
                  className="ml-4 p-2 text-gray-500 hover:text-gray-700 hover:bg-amber-100 rounded-lg transition-colors"
                >
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-base font-semibold text-gray-900">전체 설교 내용</h4>
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(selectedSermon.fullText)
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 max-h-96 overflow-y-auto">
                        <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">
                          {selectedSermon.fullText}
                        </p>
                      </div>
                    </div>
                    ...
                    <div>
                      <h4 className="text-base font-semibold text-gray-900 mb-3">
                        청크별 보기 ({selectedSermon.totalChunks}개)
                      </h4>
                      <div className="space-y-3">
                        {selectedSermon.chunks.map((chunk, idx) => (
                          <div
                            key={idx}
                            className="border border-amber-100 rounded-lg p-3 bg-white hover:bg-amber-50/50 transition-colors"
                          >
              ...
              {/* 모달 푸터 */}
              {selectedSermon?.sermon && (
                <div className="px-6 py-3 border-t border-amber-100 bg-amber-50/50 flex items-center justify-between">
                  <a
                    href={selectedSermon.sermon.videoUrl}
                    target="_blank"
                    rel="noopener noreferrer"
